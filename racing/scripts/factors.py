...
def callback_control(data):
    ...
    if obstacle:
        if location == 2:
            vxf = (
                vx * k_c_x
                + data.data[0] * math.sin(0.349) * k_u
                + data.data[0] * math.sin(angle) * k_s_x
            )
        else:
            vxf = (
                vx * k_c_x
        ...
        vxf = vx * k_c_y + data.data[0] * math.sin(angle) * k_s_y
    vyf = vy * k_c_y + data.data[0] * math.cos(angle) * k_s_y
